...
class GroqProvider(LLMProvider):
    ...
    def complete(self, prompt: str, system: str = None) -> str:
        messages = []
        # Groq requires 'json' in prompt for json_object
        sys_msg = f"{system} (respond in JSON format)" if system else "Respond in JSON format"
        ...
        for attempt in range(5):
            try:
                ...
                if e.code == 429:
                    print_warn(f"Rate limited by Groq. Retrying in {(attempt+1)*2}s...")
...
def create_provider() -> LLMProvider:
    ...
    providers = {
        "xai": lambda: XAIProvider(LLM_API_KEY, LLM_MODEL),
